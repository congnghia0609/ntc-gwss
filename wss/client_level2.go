...
func (c *ClientLevel2) respMsg(message string) {
	util.TCF{
		Try: func() {
			if len(message) > 0 {
				msg := []byte(message)
		...
		Catch: func(e util.Exception) {
			log.Printf("ClientLevel2.respMsg Caught %v\n", e)
		},
		Finally: func() {
			//log.Println("Finally...")
		},
...
// serveWsLevel2 handles websocket requests from the peer.
func serveWsLevel2(symbol string, typetime string, hub *HubLevel2, w http.ResponseWriter, r *http.Request) {
	util.TCF{
		Try: func() {
			...
			client := &ClientLevel2{symbol: symbol, typeTime: typetime, hub: hub, conn: conn, send: make(chan []byte, sendBuffer)}
		...
		Catch: func(e util.Exception) {
			log.Printf("serveWsLevel2 Caught %v\n", e)
		},
		Finally: func() {
			//log.Println("Finally...")
		},
